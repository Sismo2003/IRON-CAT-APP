...
const TopSellingProducts = () => {
  ...
  // Estados para paginación - Cambiado a 2 transportes por página
  const [paginaActual, setPaginaActual] = useState(1);
  const transportesPorPagina = 2;
  ...
  // Función para guardar cambios - Modificada para redirigir a nueva página
  const handleGuardar = () => {
    if (agregando) {
      ...
      const nuevosTransportes = [...transportes, nuevoTransporte];
      setTransportes(nuevosTransportes);
      
      // Calcular nueva página y redirigir
      const nuevaPagina = Math.ceil(nuevosTransportes.length / transportesPorPagina);
        ...
        <div className="card-body">
          <div className="flex items-center mb-3">
            ...
            <Dropdown className="relative shrink-0">
              <Dropdown.Trigger type="button" className="flex items-center justify-center size-[30px] p-0 bg-white text-slate-500 btn hover:text-slate-500 hover:bg-slate-100 focus:text-slate-500 focus:bg-slate-100 active:text-slate-500 active:bg-slate-100" id="sellingProductDropdown">
                <MoreVertical className="inline-block size-4" />
              </Dropdown.Trigger>
              <Dropdown.Content placement="right-end" className="absolute z-50 py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem]" aria-labelledby="sellingProductDropdown">
                <li>
                  <button onClick={handleIniciarAgregar} className="flex items-center w-full px-4 py-1.5 text-base text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500">
                    <Plus className="size-4 mr-2" /> Agregar
                  </button>
                </li>
                <li>
                  <button onClick={generarReporteGeneral} className="flex items-center w-full px-4 py-1.5 text-base text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500">
          ...
          <div>
            <ul className="flex flex-col gap-5">
              {transportesActuales.map((transporte) => (
                <li key={transporte.id} className="flex items-center gap-3 p-3 border rounded-md border-slate-200">
                  <div className="flex items-center justify-center size-10 rounded-md bg-slate-100">
                  ...
                  <div className="overflow-hidden grow">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
                      ...
                      <div>
                        <span className="text-slate-500">Adeudos:</span> 
                    ...
                    <div className="mt-2">
                      ...
                      <p className="text-sm text-slate-600 line-clamp-1">{transporte.novedades}</p>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-4xl p-6 bg-white rounded-lg shadow-lg">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setMostrarReporte(false)} className="p-1 text-slate-500 hover:text-slate-700">
                <X className="size-5" />
              </button>
            </div>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {transportes.map((transporte) => (
                    <tr key={transporte.id} className="border-b">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 border rounded-md border-slate-200"
            ...
            <div>
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-slate-500">Placa: {transporteSeleccionado.placa}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 mb-6 md:grid-cols-2">
                <div className="p-3 border rounded-md border-slate-200">
                  ...
                  <div className="space-y-2">
                    <p><span className="text-slate-500">Conductor:</span> {transporteSeleccionado.conductor}</p>
                    <p><span className="text-slate-500">Kilometraje:</span> {transporteSeleccionado.kilometraje}</p>
                    <p>
                      <span className="text-slate-500">Adeudos:</span> 
                ...
                <div className="p-3 border rounded-md border-slate-200">
                  ...
                  <p className="text-slate-600">{transporteSeleccionado.novedades}</p>
                </div>
              </div>
