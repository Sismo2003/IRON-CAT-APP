...
const TicketList = () => {
    ...
    const handleDelete = () => {
        if (eventData) {
            dispatch(onDeleteTicket(eventData.id));
    ...
    const validation: any = useFormik({
        ...
        onSubmit: (values) => {
            if (isEdit) {
                ...
                // update user
                dispatch(onUpdateTicket(updateData));
            } else {
                const newData = {
                    ...values,
    ...
    return (
        <React.Fragment>
            <BreadCrumb title='Lista de Tickets' pageTitle='Administración Tickets' />
            <DeleteModal show={deleteModal} onHide={deleteToggle} onDelete={handleDelete} />
            <ToastContainer closeButton={false} limit={1} />
            <div className="card" id="employeeTable">
                <div className="card-body">
                    <div className="flex items-center gap-3 mb-4">
                        <h6 className="text-15 grow">Tickets (<b className="total-Employs">{data.length}</b>)</h6>
